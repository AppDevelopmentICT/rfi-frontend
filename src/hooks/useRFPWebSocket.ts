...
function buildWsUrl(): string {
  ...
  return `${wsBase}/api/v1/rfp/ws/generate-technical?token=${encodeURIComponent(token)}`;
}
...
export function useRFPWebSocket(options?: UseRFPWebSocketOptions) {
  ...
  const connect = useCallback(() => {
    ...
    console.log("[RFP WS] Connecting to", url);
  ...
  const generate = useCallback(
    (product: string, extras?: { projectName?: string; projectDescription?: string; additionalContext?: string }) => {
